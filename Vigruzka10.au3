...
; файлы которые должны быть выгружены
Dim $FilesMain[0]
;_ArrayAdd($FilesMain, "X:\00_BIM\14_Академика Волгина\04_OT\AV_04_OT_PPFK_RD_R19_UND.rvt")
;_ArrayAdd($FilesMain, "X:\00_BIM\14_Академика Волгина\04_OT\AV_04_OT_PPFK_RD_R19_UND.rvt")
;_ArrayAdd($FilesMain, "X:\00_BIM\14_Академика Волгина\04_OT\AV_04_OT_PPFK_RD_R19_UND.rvt")
...
;$LogFile="C:\ForPlanirovshik" используется указаная папка , имя будет совпадать с иполняемым фалом только в конце добавляется "_log.txt"
...
Func ExportNwc($MainF , $FolderNW , $LogFile , $ExcludeArr , $FilesMain)
	...
	if UBound( $FilesMain ) > 0 then
		_ArrayConcatenate($FileList2, $FilesMain)
	EndIf
...
ExportNwc($MainF , $FolderNW , $LogFile , $ExcludeArr , $FilesMain)
